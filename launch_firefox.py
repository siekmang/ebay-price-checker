...
def launch_firefox():
    ...
    for i in range(2, 5):
        wait.until(ec.presence_of_element_located((By.XPATH, ".//span[contains(@class, 'su-styled-text primary default')]")))
        title_element = results[i].find_element(By.XPATH, ".//span[contains(@class, 'su-styled-text primary default')]")

        price_elements = results[i].find_elements(By.XPATH, ".//span[@class='su-styled-text primary bold large-1 s-card__price']")
